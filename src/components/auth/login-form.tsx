...
export function LoginForm({ onSuccess, onSwitchToSignUp }: LoginFormProps) {
  ...
  const { signIn, signInWithProvider, isLoading } = useAuthContext();
  ...
  const onSubmit = async (data: LoginFormData) => {
    ...
    if (result.success) {
      ...
      // Set form-level error for auth failures
      setError('root', {
        type: 'manual',
        message: (result.error as { message?: string })?.message || 'Failed to sign in',
      });
    }
  };
